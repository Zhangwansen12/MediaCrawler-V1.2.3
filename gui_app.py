...
class MediaCrawlerGUI:
    ...
    def __init__(self):
        ...
        self.platform_mapping = {
            "小红书": "xhs", "抖音": "dy", "快手": "ks", 
            "哔哩哔哩": "bili", "微博": "wb", "贴吧": "tieba", "知乎": "zhihu"
        }
        self.login_type_mapping = {
            "二维码": "qrcode", "手机号": "phone", "Cookie": "cookie"
        }
        self.crawler_type_mapping = {
            "搜索": "search", "详情": "detail", "创作者": "creator", "商城": "mall"
        }
        self.save_option_mapping = {
            "JSON": "json", "CSV": "csv", "SQLite": "sqlite", "数据库": "db"
        }
        self.mall_function_mapping = {
            "商品搜索": "product_search", "热门商品": "trending_products", 
            "商品详情": "product_detail", "数据分析": "analytics"
        }
        self.sort_type_mapping = {
            "销量": "sales", "价格升序": "price_asc", "价格降序": "price_desc", 
            "评分": "rating", "最新": "newest"
        }
        self.metric_mapping = {
            "点赞数": "likes", "评论数": "comments", "分享数": "shares", 
            "收藏数": "collects", "播放数": "views", "互动率": "engagement_rate"
        }
    ...
    def auto_resize_window(self):
        ...
        # 计算新的窗口尺寸（不超过屏幕的80%）
        new_width = min(max(required_width + 50, current_width), int(screen_width * 0.9))
        new_height = min(max(required_height + 50, current_height), int(screen_height * 0.9))
        ...
        if new_width != current_width or new_height != current_height:
            ...
            if new_width > current_width:
                x = max(0, x - (new_width - current_width) // 2)
            if new_height > current_height:
                y = max(0, y - (new_height - current_height) // 2)
            
            # 确保窗口不超出屏幕边界
            x = min(x, screen_width - new_width)
            y = min(y, screen_height - new_height)
    ...
    def create_basic_config(self, parent):
        ...
        platform_combo = ttk.Combobox(config_frame, textvariable=self.platform_var, 
                                     values=["小红书", "抖音", "快手", "哔哩哔哩", "微博", "贴吧", "知乎"],
        ...
        self.crawler_type_var = tk.StringVar(value="搜索")
    ...
    def create_crawler_functions(self, parent):
        ...
        # 第三行按钮（抖音热搜相关）
        row3_frame = ttk.Frame(crawler_frame)
    ...
    def create_log_panel(self, parent):
        ...
        # 初始化日志
        self.log_message("MediaCrawler 图形界面已启动")
    ...
    def start_crawling(self):
        ...
        # 将中文配置值转换为英文值
        platform_en = self.platform_mapping.get(self.platform_var.get(), self.platform_var.get())
        login_type_en = self.login_type_mapping.get(self.login_type_var.get(), self.login_type_var.get())
        crawler_type_en = self.crawler_type_mapping.get(self.crawler_type_var.get(), self.crawler_type_var.get())
        save_option_en = self.save_option_mapping.get(self.save_option_var.get(), self.save_option_var.get())
        ...
        cmd = [
            ...
            "--lt", login_type_en,
            "--type", crawler_type_en,
            "--save_data_option", save_option_en,
        ...
        if platform_en == "ks" and hasattr(self, 'enable_video_stats_var'):
        ...
        def run_crawler():
            try:
                ...
                for line in iter(self.current_process.stdout.readline, ''):
                    if line:
                        self.log_message(line.strip(), "爬虫")
                
                self.current_process.wait()
                
                if self.current_process.returncode == 0:
                    self.log_message("爬取完成", "成功")
                else:
                    self.log_message(f"爬取失败，退出码: {self.current_process.returncode}", "错误")
                    
            except Exception as e:
                self.log_message(f"爬取过程出错: {str(e)}", "错误")
            finally:
                self.is_running = False
                self.current_process = None
        ...
        def run_init():
            try:
                ...
                if result.returncode == 0:
                    self.log_message(f"{db_type.upper()}数据库初始化成功", "成功")
                    messagebox.showinfo("成功", f"{db_type.upper()}数据库初始化完成")
                else:
                    self.log_message(f"数据库初始化失败: {result.stderr}", "错误")
                    messagebox.showerror("错误", f"数据库初始化失败:\n{result.stderr}")
            except Exception as e:
                self.log_message(f"数据库初始化出错: {str(e)}", "错误")
    ...
    def start_sms_server(self):
        ...
        def run_sms_server():
            try:
                ...
                self.log_message("短信服务器已启动，监听端口8000", "成功")
            except Exception as e:
                self.log_message(f"启动短信服务器失败: {str(e)}", "错误")
...
项目地址：https://github.com/NanmiCoder/MediaCrawler
        ...
        if result:
            try:
                ...
                if os.path.exists(data_dir):
                    ...
                    self.log_message("数据清理完成", "成功")
                    messagebox.showinfo("成功", "数据清理完成")
                else:
                    messagebox.showinfo("提示", "数据目录不存在")
            except Exception as e:
                self.log_message(f"数据清理失败: {str(e)}", "错误")
    ...
    def save_log(self):
        ...
        if filename:
            try:
                ...
                self.log_message(f"日志已保存到: {filename}", "成功")
                messagebox.showinfo("成功", "日志保存成功")
            except Exception as e:
                self.log_message(f"日志保存失败: {str(e)}", "错误")
    ...
    def on_platform_changed(self, event=None):
        ...
        # 将中文平台名转换为英文
        platform_en = self.platform_mapping.get(platform, platform)
        ...
        # 界面元素变化后自动调整窗口大小
        self.root.after(50, self.auto_resize_window)
    
    # 删除了create_douyin_hot_search_config方法

    # 删除了所有热搜相关方法：
    # - fetch_hot_search_data
    # - _call_hot_search_api
    # - _get_mock_hot_search_data
    # - _try_real_api
    # - _update_hot_search_results
    # - _save_hot_search_data
    # - open_hot_search_window
    ...
    def on_crawler_type_changed(self, event=None):
        ...
        platform = self.platform_var.get()
        # 将中文值转换为英文
        crawler_type_en = self.crawler_type_mapping.get(crawler_type, crawler_type)
        platform_en = self.platform_mapping.get(platform, platform)
        
        # 显示或隐藏小红书商城配置
        if hasattr(self, 'xhs_mall_frame'):
            if platform_en == "xhs" and crawler_type_en == "mall":
    ...
    def create_kuaishou_stats_config(self, parent):
        ...
        self.top_videos_metric_var = tk.StringVar(value="点赞数")
        self.stats_display_language_var = tk.StringVar(value="中文")
        ...
        metric_combo = ttk.Combobox(self.kuaishou_stats_frame, textvariable=self.top_videos_metric_var,
                                   values=["点赞数", "评论数", "分享数", "收藏数", "播放数", "互动率"],
        ...
        lang_combo = ttk.Combobox(self.kuaishou_stats_frame, textvariable=self.stats_display_language_var,
                                 values=["中文", "英文"], state="readonly", width=8)
    ...
    def update_kuaishou_config(self):
        ...
        try:
            ...
            # 将中文配置值转换为英文值
            metric_en = self.metric_mapping.get(self.top_videos_metric_var.get(), self.top_videos_metric_var.get())
            language_en = self.language_mapping.get(self.stats_display_language_var.get(), self.stats_display_language_var.get())
            ...
            config_updates = {
                ...
                'TOP_VIDEOS_METRIC': f'"{metric_en}"',
                'STATS_DISPLAY_LANGUAGE': f'"{language_en}"'
            }
            ...
            self.log_message("快手统计功能配置已更新", "成功")
            
        except Exception as e:
            self.log_message(f"更新快手配置失败: {str(e)}", "错误")
    
    def open_xhs_mall(self):
        """打开小红书商城数据管理界面"""
        try:
            # 将中文配置值转换为英文值
            mall_function_en = self.mall_function_mapping.get(
                getattr(self, 'mall_function_var', tk.StringVar(value="商品搜索")).get(), 
                "product_search"
            )
            sort_type_en = self.sort_type_mapping.get(
                getattr(self, 'sort_type_var', tk.StringVar(value="销量")).get(), 
                "sales"
            )
            
            # 获取当前配置
            config = {
                'mall_function': mall_function_en,
            ...
            self.log_message("小红书商城数据管理界面已打开", "成功")
                    ...
                    try:
                        ...
                        if platform:
                            ...
                            if video_info and video_info.get('video_url'):
                                ...
                                self.log_message(f"第 {i} 个视频解析失败", "警告")
                        else:
                            self.log_message(f"第 {i} 个视频平台不支持", "警告")
                    except Exception as e:
                        self.log_message(f"第 {i} 个视频下载出错: {str(e)}", "错误")
    ...
    def select_data_file(self):
        ...
        if file_path:
            ...
            self.log_message(f"已选择数据文件: {file_path}", "信息")
...
class ContentPreviewWindow:
    ...
    def __init__(self, parent):
        ...
        # 界面创建完成后，自动调整窗口大小
        self.window.after(100, self.auto_resize_window)
    ...
    def create_interface(self):
        ...
        self.content_tree.heading('#0', text='编号')
