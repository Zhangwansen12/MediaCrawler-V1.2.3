...
class KuaiShouClient(AbstractApiClient):
    ...
    async def get_video_stats(self, photo_id: str) -> Dict:
        """
        获取视频统计数据（专门用于统计信息）
        :param photo_id: 视频ID
        :return: 包含统计数据的字典
        """
        try:
            video_info = await self.get_video_info(photo_id)
            video_detail = video_info.get("visionVideoDetail", {})
            
            if not video_detail:
                utils.logger.warning(f"[KuaiShouClient.get_video_stats] No video detail found for {photo_id}")
                return {}
            ...
            # 提取统计数据
            stats_data = {
                "video_id": photo_data.get("id", photo_id),
                ...
                "author": video_detail.get("author", {}).get("name", "")
            }
            
            utils.logger.info(
                f"[KuaiShouClient.get_video_stats] Retrieved stats for video {photo_id}: "
                f"likes={stats_data['like_count']}, comments={stats_data['comment_count']}, "
                f"shares={stats_data['share_count']}, collects={stats_data['collect_count']}"
            )
            
            return stats_data
            
        except Exception as e:
            utils.logger.error(f"[KuaiShouClient.get_video_stats] Error getting video stats for {photo_id}: {e}")
            return {}

    async def batch_get_video_stats(self, photo_ids: List[str]) -> List[Dict]:
        """
        批量获取视频统计数据
        :param photo_ids: 视频ID列表
        :return: 统计数据列表
        """
        stats_list = []
        ...
        for photo_id in photo_ids:
            try:
                stats = await self.get_video_stats(photo_id)
                if stats:
                ...
                utils.logger.error(f"[KuaiShouClient.batch_get_video_stats] Error processing {photo_id}: {e}")
                continue
        
        utils.logger.info(f"[KuaiShouClient.batch_get_video_stats] Retrieved stats for {len(stats_list)}/{len(photo_ids)} videos")
        return stats_list
