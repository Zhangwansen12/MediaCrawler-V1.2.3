    ...
    async def get_video_info_task(
        ...
        async with semaphore:
            try:
                ...
                if video_detail:
                    # 提取视频统计数据
                    video_stats = VideoStatsExtractor.extract_video_stats(video_detail)
                    if video_stats:
                        # 记录统计数据
                        utils.logger.info(
                            f"[KuaishouCrawler.get_video_info_task] Video {video_id} stats: "
                            f"likes={video_stats.like_count}, comments={video_stats.comment_count}, "
                            f"shares={video_stats.share_count}, collects={video_stats.collect_count}, "
                            f"views={video_stats.view_count}, engagement_rate={VideoStatsExtractor.get_engagement_rate(video_stats)}%"
                        )
                        
                        # 将统计数据添加到video_detail中以便后续处理
                        video_detail["extracted_stats"] = video_stats._asdict()
