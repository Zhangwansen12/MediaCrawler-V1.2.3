...
from .mall import XiaoHongShuMallManager
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    mall_manager: Optional[XiaoHongShuMallManager]
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            # Initialize mall manager for e-commerce data
            self.mall_manager = XiaoHongShuMallManager(self.xhs_client, self.context_page)
            utils.logger.info("[XiaoHongShuCrawler.start] Mall manager initialized")
            ...
            elif config.CRAWLER_TYPE == "mall":
                # Get mall product data
                await self.get_mall_products()
            else:
                pass
    ...
    async def get_note_images(self, note_item: Dict):
        ...
        for pic in image_list:
            ...
            if content is None:
                continue
            extension = ".mp4"
            file_name = f"{note_id}_{videoNum}{extension}"
            file_path = os.path.join(utils.get_videos_path(), file_name)
            with open(file_path, "wb") as f:
                f.write(content)
            videoNum += 1

    async def get_mall_products(self) -> None:
        """Get mall product data"""
        utils.logger.info("[XiaoHongShuCrawler.get_mall_products] Begin get mall products")
        
        if not self.mall_manager:
            utils.logger.error("[XiaoHongShuCrawler.get_mall_products] Mall manager not initialized")
            return
        
        try:
            # Get products by keywords from config
            for keyword in config.KEYWORDS.split(","):
                utils.logger.info(f"[XiaoHongShuCrawler.get_mall_products] Getting products for keyword: {keyword}")
                
                products = await self.mall_manager.get_products_with_processing(
                    keyword=keyword.strip(),
                    max_count=config.CRAWLER_MAX_NOTES_COUNT
                )
                
                if products:
                    utils.logger.info(f"[XiaoHongShuCrawler.get_mall_products] Found {len(products)} products for keyword: {keyword}")
                    
                    # Save products to store
                    for product in products:
                        await xhs_store.update_xhs_mall_product(product)
                    
                    # Get analytics data
                    analytics = await self.mall_manager.get_product_analytics(products)
                    if analytics:
                        utils.logger.info(f"[XiaoHongShuCrawler.get_mall_products] Analytics: {analytics}")
                        await xhs_store.update_xhs_mall_analytics(analytics)
                else:
                    utils.logger.warning(f"[XiaoHongShuCrawler.get_mall_products] No products found for keyword: {keyword}")
                
                await asyncio.sleep(1)  # Rate limiting
                
        except Exception as e:
            utils.logger.error(f"[XiaoHongShuCrawler.get_mall_products] Error getting mall products: {e}")

    async def get_mall_product_detail(self, product_id: str) -> Optional[Dict]:
        """Get detailed information for a specific product"""
        if not self.mall_manager:
            utils.logger.error("[XiaoHongShuCrawler.get_mall_product_detail] Mall manager not initialized")
            return None
        
        try:
            detail = await self.mall_manager.mall_client.get_product_detail(product_id)
            if detail:
                processed_detail = self.mall_manager.data_processor.process_product_data(detail)
                await xhs_store.update_xhs_mall_product(processed_detail)
                return processed_detail
        except Exception as e:
            utils.logger.error(f"[XiaoHongShuCrawler.get_mall_product_detail] Error getting product detail: {e}")
